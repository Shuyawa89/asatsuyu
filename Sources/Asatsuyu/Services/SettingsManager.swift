...
class SettingsManager: ObservableObject {
    ...
    private init() {
        // デフォルト値の設定
        self.workDuration = userDefaults.object(forKey: Keys.workDuration) as? TimeInterval ?? 1800 // 30分
        self.shortBreakDuration = userDefaults.object(forKey: Keys.shortBreakDuration) as? TimeInterval ?? 300 // 5分
        self.longBreakDuration = userDefaults.object(forKey: Keys.longBreakDuration) as? TimeInterval ?? 900 // 15分
        self.cyclesUntilLongBreak = userDefaults.object(forKey: Keys.cyclesUntilLongBreak) as? Int ?? 4
        
        self.soundEnabled = userDefaults.object(forKey: Keys.soundEnabled) as? Bool ?? true
        self.soundName = userDefaults.object(forKey: Keys.soundName) as? String ?? "default"
        self.bannerEnabled = userDefaults.object(forKey: Keys.bannerEnabled) as? Bool ?? true
        
        self.obsidianVaultPath = userDefaults.object(forKey: Keys.obsidianVaultPath) as? String
        self.autoSaveEnabled = userDefaults.object(forKey: Keys.autoSaveEnabled) as? Bool ?? true
        self.fileNameFormat = userDefaults.object(forKey: Keys.fileNameFormat) as? String ?? "YYYY-MM-DD"
    }
